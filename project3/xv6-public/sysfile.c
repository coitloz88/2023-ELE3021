...
// Creates and returns inode supplied in path
// (expecting path to exist up to its last part,
// which is the inode we're creating).
...
create(char *path, short type, short major, short minor)
{
  ...
  if ((ip = dirlookup(dp, name, 0)) != 0)
  {
    ...
    if ((type == T_FILE && ip->type == T_FILE) || (type == T_SYMLINK && ip->type == T_SYMLINK))
      return ip;
    // symbolic link를 만드는 것과 file을 만드는 동작은 유사..?
...
int sys_open(void)
{
  ...
  repeatSymlink:
    ...
    // 열려는 파일이 directory인데 write하려고 하면 에러
...
int sys_symlink(void)
{
  ...
  // TODO: symbolic link는 hard link와 다르게 inode를 새롭게 만들어야한다.
  // 그러므로, namei를 통해 old path의 inode를 가져오는 것이 아니라, create를 통해 inode를 만들어줘야함
...
int sys_readlink(void)
{
  ...
  safestrcpy(buffer, (char *)ip->addrs, bufsize);
